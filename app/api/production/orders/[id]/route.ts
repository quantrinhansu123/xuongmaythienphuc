...
export async function GET(
    ...
    try {
        ...
        // Get production order details with item info
      ...
      WHERE po.id = $1`,
        ...
        // Construct the single item object
        const item = {
            id: productionOrder.orderDetailId,
        ...
        // Fetch measurements for this specific item
        if (item.id) {
         ...
         WHERE om.order_detail_id = $1`,
                [item.id]
            );

            item.measurements = measurementsResult.rows;
        }
