...
export async function PUT(
  ...
  try {
    ...
    let { categoryName, parentId, description, type } = body;
    ...
    const result = await query(
      `UPDATE item_categories 
       SET category_name = $1, parent_id = $2, description = $3, type = $4, updated_at = CURRENT_TIMESTAMP
       WHERE id = $5
       RETURNING id, category_code as "categoryCode", category_name as "categoryName", type`,
      [categoryName, finalParentId, description || null, type || 'PRODUCT', id]
    );
